...
def train_cptn():
    ...
    # load dataset
    trainset = tf.data.TFRecordDataset(join('datasets', 'trainset.tfrecord')).repeat(-1).map(ctpn_parse_function).batch(
    ...
    if not exists('result_model'):
        mkdir('result_model')
    detector.ctpn.save(join('result_model', 'my_ctpn.h5'))
...
def train_ocr():
    ...
    # load dataset
    trainset = tf.data.Dataset.from_generator(generator.gen, (tf.float32, tf.int64),
                                              (tf.TensorShape([32, None, 3]), tf.TensorShape([None, ]))).repeat(-1).map(
        ocr_parse_function).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
    # restore from existing checkpoint
    if not exists('checkpoints'):
        mkdir('checkpoints')
    ...
    for image, labels in trainset:
        ...
        with tf.GradientTape() as tape:
            ...
            if tf.math.reduce_any(tf.math.is_nan(pred)):
                ...
                continue
            loss = tf.keras.backend.ctc_batch_cost(y_true=labels, y_pred=pred,
        ...
        if tf.equal(optimizer.iterations % 100, 0):
            with log.as_default():
                ...
                tf.summary.image('image', tf.cast((image[0:1, ...] / 2 + 0.5) * 255., dtype=tf.uint8),
